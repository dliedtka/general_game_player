...
function play (id,move)
 {if (move!=='nil') {state = simulate(doesify(roles,move),state,library)};
  if (roles.length > 1) {
    return bestmovemulti(role, state, library, [1]);
  } else {
    return bestmovesingle(role, state, library, [1]);
  }}

function bestmovesingle(role, state, library, level){
var actions = findlegals(role, state, library);
  var action = actions[0];
  var score = 0;
  for (var i=0; i<actions.length; i++){
    level[0] += 1;
    var result = maxscoresingle(role, simulate([actions[i]], state, library), library, level);
...
function bestmovemulti(role, state, library, level){
...
function minscore(role, action, state, library, level){
	...
	var result = maxscoremulti(role, newstate, library, level);
	// bound at min score
	if (result == 0){
	    return 0;
	}
